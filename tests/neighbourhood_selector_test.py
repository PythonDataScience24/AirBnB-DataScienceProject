...
from src.neighbourhood_selector import NeighbourhoodSelector
import pandas as pd


class NeighbourhoodSelectorTest(unittest.TestCase):
    """Test class for NeighbourhoodSelector"""
    def setUp(self):
        table = {'id': [1, 2, 3, 4, 5],
                 'name': ['Room with a View', 'Loft', 'Cheap', 'Cozy', 'Expensive'],
                 'neighbourhood_group': ['Manhattan', 'Manhattan', 'Brooklyn', 'Brooklyn',
                                         'Queens'],
                 'neighbourhood': ['Upper East Side', 'East Village', 'Williamsburg',
    ...
    def test_set_selection_neighbourhood(self):
        ...
        self.assertEqual(df['name'].to_list(), ['Cheap', 'Cozy'],
                         msg="Neighbourhood selection failed")
    ...
    def test_set_selection_room_type(self):
        ...
        self.assertEqual(df['name'].to_list(), ['Cheap', 'Expensive'],
                         msg="Room type selection failed")
    ...
    def test_set_selection_price(self):
        ...
        self.assertEqual(df['name'].to_list(), ['Room with a View', 'Cheap', 'Cozy'],
                         msg="Price selection failed")
    ...
    def test_set_selection_combined(self):
        ...
        df = self.neighbourhood_selector.set_selection(neighbourhood='Williamsburg',
                                                       ...
                                                       price=100)
        self.assertEqual(df['name'].to_list(), ['Cheap'],
                         msg="Neighbourhood and room type selection failed")

    def test_set_selection_impossible(self):
        """Test if the selection of an impossible combination works correctly"""
        df = self.neighbourhood_selector.set_selection(neighbourhood='Williamsburg',
                                                       room_type='Private room', price=0)
    ...
    def test_set_selection_nonexistent_neighbourhood(self):
        ...
        self.assertEqual(df, None, msg="Neighbourhood selection should be None")

    def test_get_neighbourhoods(self):
        """Test if all the neighbourhoods are correctly returned"""
        self.assertEqual(self.neighbourhood_selector.get_neighbourhoods(),
